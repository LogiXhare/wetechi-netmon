...
| FU-42 | Six Phase 5B-1 dependency candidates are "Conditionally Accepted," not yet added | Milestone 5B-1 | `uuid` ([ADR 0019](../architecture/decisions/0019-phase5b-uuidv7-identity-generation.md)), `tokio-postgres` ([ADR 0020](../architecture/decisions/0020-phase5b-postgresql-client.md)), `deadpool-postgres` ([ADR 0022](../architecture/decisions/0022-phase5b-connection-pool.md)), `rustls`/`tokio-postgres-rustls` ([ADR 0023](../architecture/decisions/0023-phase5b-postgresql-tls.md)), and `refinery` ([ADR 0024](../architecture/decisions/0024-phase5b-migration-framework.md)) all have verified version/license/advisory evidence (2026-08-24, see [dependency-license-matrix.md](../dependency-license-matrix.md) rows 32–37) but no measured `cargo tree`, `cargo audit`, `unsafe` inventory, or Windows-GNU + Linux build result — per [ADR 0018](../architecture/decisions/0018-phase5-dependency-selection.md)'s honesty constraint, measuring these requires actually adding the dependency, which is exactly why Milestone 5B-1 exists as its own reversible step before 5B-2 onward depends on the result. **Acceptance gate:** all six probe results recorded, matrix rows updated from "Conditionally Approved" to "Approved" or "Rejected," `NOTICE` updated where a license requires attribution, before any 5B-2 schema work begins. |
| FU-43 | Phase 5B-0's repository/unit-of-work seam extraction is a refactor of code two Opus 5 adversarial reviews already certified (0 Blocker, 0 High as of the PR #16 merge) | Milestone 5B-0, in progress | Extracting a trait from `IncidentUnitOfWork`'s direct storage accesses (verified count at 5B-0 start: 93, not the 72 ADR 0029 states — recorded here as a correction, not a re-count later found different), and moving the FU-38 guard hardening into the same pass, is real surgery on recently-certified code — see [ADR 0029](../architecture/decisions/0029-phase5b-repository-and-unit-of-work-seam.md) and **R20** in [risk-register.md](../risk-register.md). **Acceptance gate:** all 531 existing Phase 5A tests remain green throughout 5B-0, proving the refactor changed no behavior, before 5B-1 begins. **Progress (2026-09-03):** durable time (ADR 0031), `IncidentSnapshot`/`reconstitute` (ADR 0030), and the FU-38 guard hardening are done — 582 of the original 531 tests pass (0 failed). The repository-trait extraction itself (ADR 0029 decision 1: introducing the trait; decision 5: migrating `IncidentUnitOfWork` to it) is **not yet done**. A fine-grained CRUD trait mirroring the current field layout (one method per `HashMap`/`Vec` operation) is the mechanically safest shape to extract, but is also the shape a 2026-08-24 Stage-A design pass flagged as wrong for a real PostgreSQL adapter to implement: individually-awaited per-collection writes would reopen 5A's own partial-write gap at the SQL level. The safer shape — a persistence-neutral mutation plan the domain computes and a caller commits atomically — is larger surgery; that same design pass recommended landing its type design in 5B-0 and deferring full call-site wiring to 5B-3. Left for explicit owner direction rather than defaulted, unlike the other 5B-0 items above. |
