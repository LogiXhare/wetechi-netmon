...
| FU-34 | `policy_refs` is written once at creation and never updated | **Fixed 2026-08-24** | `link_event_to_open_incident` now updates the matching `PolicyRef`'s `last_seen_sequence`/`policy_version` on every linked event, or appends a new `PolicyRef` for a previously-unseen `policy_id` (bounded by `POLICY_REFS_MAX`, matching the evidence ledger's stops-growing-keeps-counting asymmetry rather than refusing the whole event link). Regression: `unit_of_work::tests::a_later_event_under_a_different_policy_is_recorded_in_policy_refs`. |
